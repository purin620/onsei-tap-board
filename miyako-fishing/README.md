...
- [~] Phase 6: 分析・可視化ダッシュボード（`analyze_trends.py`を先行実装。下記「Phase 6について」参照）
...
## Phase 6について（傾向分析・釣行日の提案、先行実装）

ユーザーから「釣果のいい日の傾向をまとめて、次に釣りに行くならいつがいいか提案してほしい」
という要望があり、本来はPhase 5(AI抽出)完了後に着手する予定だったPhase 6を先行して
一部実装した(2026-09-21)。`analyze_trends.py`が本体。

### できること

- `tendency_by_tide(species)`: ヒラメなど、潮回り(大潮/中潮/小潮)ごとの釣果スコアの平均を集計
- `tendency_by_water_temp(species)`: イカなど、水温帯ごとの釣果スコアの平均を集計
- `recommend_next_days(species)`: Open-Meteoの気温予報(`fetch_weather.fetch_weather_forecast()`、
  今回追加)を取得し、過去の好釣果日の気温傾向に近い日から順に提案する

釣果の記録は数値の匹数(ヒラメ想定)と豊漁/普通/不漁(イカ想定)が混在するため、
`score_from_count_or_level()`で豊漁=3・普通=2・不漁=1、数値はそのまま、という
共通スコアに変換してから集計している。

### 重要な制約（今の時点ではまだ実用的な提案はできない）

- `extracted`テーブルの中身はPhase 1で手入力したごく少数のダミーデータしかなく
  (2026-09-21時点でヒラメ2件・イカ1件)、統計的に意味のある傾向はまだ出せない。
  そのため`MIN_SAMPLES_FOR_TENDENCY`(5件)未満のときは`enough_data: false`を返し、
  断定的な提案をしないようにしてある。実際に`python analyze_trends.py`を実行すると
  「データが足りません」という結果になる。
- `recommend_next_days()`は本来、ヒラメなら潮回りの予報、イカなら水温の予報と
  比較すべきだが、潮回りの将来予測(tide736.net、港コード未確認)も水温の将来予測
  (情報源自体が未確定)もまだ使えない。そのため現状は**気温予報のみ**を手がかりにした
  暫定的な提案にとどまる。この制約は関数のdocstringと戻り値の`note`にも明記している。
- Phase 3・4のスクレイパーが実際に集めた実データと、Phase 5のAI抽出が揃ってくれば、
  この分析はそのまま実用的な提案に育つように設計してある(スコア集計・傾向分析の
  ロジック自体は`test_analyze_trends.py`のオフラインテストで動作確認済み)。

### 実行例

```bash
python test_analyze_trends.py       # オフラインでのロジック検証
python analyze_trends.py            # 今のDBの中身で傾向・提案を試す(データ不足の結果になる)
```

## 次にやること

こうしん丸・隆勝丸の実際のページ構造を確認し、それぞれ専用のスクレイパーを実装する。
あわせて、Phase 5(AI抽出)を実装してデータ件数を増やし、Phase 6の分析を実用的にしていく。
